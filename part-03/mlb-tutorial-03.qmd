...
2.  **Echelon Stock Berechnung:** Angenommen, am Ende von Woche 4 sind die physischen Lagerbestände wie folgt: `L1: 10, S1: 5, P1: 15, W1: 20`. Berechnen Sie für diesen Zeitpunkt den **Echelon Stock** (systemweiten Bestand) für das Wifi-Modul W1 und das LED-Panel P1.
...
*   **Grundlogik (Stufe für Stufe):**
    1.  **Bruttobedarf ermitteln:** Für Endprodukte ist dies der Primärbedarf. Für Komponenten ergibt er sich aus den *geplanten Bestellungen* (Netto-Bedarf mit Vorlaufzeit) der übergeordneten Stufe.
    2.  **Nettobedarf berechnen:** `Nettobedarf_t = Bruttobedarf_t - Lagerbestand_am_Anfang_der_Periode_t`. Wenn das Ergebnis negativ oder null ist, ist der Nettobedarf 0.
    3.  **Bestelltermin planen:** Der Auftrag zur Deckung des Nettobedarfs in Periode `t` muss in Periode `t - Vorlaufzeit` ausgelöst werden.
*   **Stück-für-Stück (Lot-for-Lot):** Die Bestellmenge entspricht exakt dem Nettobedarf. Es wird kein Lagerbestand zur Losgrößenoptimierung aufgebaut.
