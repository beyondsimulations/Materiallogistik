...
1.  **Wareneingang:** Zu Beginn der Woche kommt eine eventuell vor $L=3$ Wochen getätigte Bestellung an. Dadurch steigt der physische Bestand und der Bestellbestand sinkt.
...
*   **Sicherheitsfaktor ($z$):** Dieser Wert hängt vom gewünschten $\alpha$-Servicegrad (Zyklus-Servicegrad) ab und wird aus der Standardnormalverteilung abgelesen: 
    *   $z = \Phi^{-1}(\alpha)$, wobei $\Phi^{-1}$ die inverse kumulative Verteilungsfunktion ist (auch Quantilfunktion genannt).

    Man kann den z-Wert in einer z-Tabelle (Standardnormalverteilungstabelle) nachschlagen. In der Tabelle sucht man den Wert, der der gewünschten Wahrscheinlichkeit $\alpha$ am nächsten kommt. Hier sind einige gebräuchliche Werte:

    | Servicegrad ($\alpha$) | z-Wert |
    | :--- | :--- |
    | 90% (0.90) | 1.282 |
    | 95% (0.95) | 1.645 |
    | 99% (0.99) | 2.326 |
...
*   **Standardisierte Verlustfunktion ($G_u(z)$):** $G_u(z) = \phi(z) - z(1 - \Phi(z))$
    ...
    *   $\Phi(z)$: Kumulative Verteilungsfunktion der Standardnormalverteilung. 
    ...
    Hier einige gebräuchliche z-Werte für die Verlustfunktion $G_u(z)$:
...
Wir müssen alle möglichen Kombinationen der Nachfrage an Tag 1 ($D_1$) und Tag 2 ($D_2$) betrachten. Die Gesamtnachfrage ist $Y_2 = D_1 + D_2$.
...
-   **P(Y2 = 2):** Wenn ($D_1=0, D_2=2$), ($D_1=1, D_2=1$), oder ($D_1=2, D_2=0$).
...
Ein Fehlbestand tritt auf, wenn die Nachfrage $Y_2$ den Bestellpunkt $s=3$ übersteigt.
$P(\text{Fehlbestand}) = P(Y_2 > 3) = P(Y_2 = 4)$
