...
Ein Fahrradhersteller in Münster plant die Produktion seines neuen E-Bike-Modells "UrbanCruiser". Die Endmontage des Fahrrads erfordert verschiedene Komponenten, deren Beziehungen in der folgenden Stückliste (Bill of Materials, BOM) dargestellt sind.
...
1.  Zeichnen Sie die Produktstruktur als Gozinto-Graph.
2.  Führen Sie eine Materialbedarfsermittlung durch. Berechnen Sie den Brutto- und Nettobedarf für jede Komponente.
3.  Bestimmen Sie die Bestellzeitpunkte für die Komponenten, um den Produktionsplan einzuhalten.
...
Ein Online-Shop für Kaffeebohnen möchte das Verbrauchsmuster einer speziellen Sorte "Äthiopischer Yirgacheffe" analysieren, um die Lagerhaltung zu optimieren. Die Verkaufszahlen (in kg) der letzten 12 Wochen liegen vor:

`[5, 8, 6, 25, 7, 4, 9, 5, 30, 6, 7, 8]`

**Ihre Aufgaben:**

1.  Stellen Sie die Zeitreihe grafisch dar.
2.  Berechnen Sie den durchschnittlichen wöchentlichen Bedarf.
3.  Berechnen Sie die mittlere absolute Abweichung (MAD), um die Prognosegüte bei Annahme eines konstanten Verbrauchs (Mittelwert) zu bewerten.
4.  Berechnen Sie den Störpegel (Variationskoeffizient).
5.  Klassifizieren Sie das Bedarfsmuster basierend auf Ihren Berechnungen. Handelt es sich um einen konstanten, trendförmigen, sporadischen oder unregelmäßigen Bedarf? Begründen Sie Ihre Entscheidung.
...
# MAD = (1/n) * sum(|y_t - mean(y)|)
mad = np.mean(np.abs(sales_data - mean_demand))

# 4. Störpegel (Variationskoeffizient)
# VK = Standardabweichung / Mittelwert
std_dev = np.std(sales_data, ddof=0) # Populations-Standardabweichung
vk = std_dev / mean_demand

print(f"2. Durchschnittlicher Bedarf: {mean_demand:.2f} kg/Woche")
print(f"3. Mittlere Absolute Abweichung (MAD): {mad:.2f} kg")
print(f"   Standardabweichung: {std_dev:.2f} kg")
print(f"4. Störpegel (Variationskoeffizient): {vk:.2f}")
...
- **Kennzahlenanalyse:**
    - Der **durchschnittliche Bedarf** liegt bei 9.67 kg.
    - Die **mittlere absolute Abweichung (MAD)** von 6.31 kg ist im Verhältnis zum Mittelwert recht hoch. Sie zeigt, dass die einzelnen Werte im Schnitt stark vom Mittelwert abweichen.
    - Der **Störpegel (Variationskoeffizient)** ist mit 0.83 ein entscheidender Indikator. In der Literatur werden oft folgende Grenzen verwendet:
        - VK < 0.25: Konstanter/glatter Bedarf
        - 0.25 <= VK <= 0.5: Schwankender, aber noch regelmäßiger Bedarf
        - VK > 0.5: Stark unregelmäßiger (erratischer) oder "lumpy" Bedarf

**Fazit:**
Mit einem Störpegel von 0.83 liegt ein **stark unregelmäßiger (erratischer) Bedarf** vor. Die Nachfrage ist schwer prognostizierbar. Die beiden hohen Spitzen deuten auf zufällige, große Bestellungen hin. Ein einfacher Prognoseansatz wie der gleitende Durchschnitt oder die exponentielle Glättung würde hier wahrscheinlich zu hohen Prognosefehlern führen. Für solche Artikel sind Sicherheitsbestände oft die bessere Strategie als eine verbrauchsgesteuerte Disposition. Da es keine Perioden mit Null-Bedarf gibt, ist die Klassifizierung "sporadisch" weniger passend als "erratisch" oder "lumpy".

:::

## Aufgabe 3: ABC-Analyse

Ein Unternehmen möchte seine Lagerartikel klassifizieren, um die Management-Anstrengungen zu priorisieren. Die folgenden Daten für 10 Artikel sind verfügbar:
...
1.  Berechnen Sie für jeden Artikel den jährlichen Verbrauchswert.
2.  Erstellen Sie eine Rangliste der Artikel nach ihrem Verbrauchswert in absteigender Reihenfolge.
3.  Berechnen Sie den prozentualen Anteil jedes Artikels am Gesamtverbrauchswert und an der Gesamtmenge aller Artikel.
4.  Berechnen Sie die kumulierten prozentualen Anteile am Wert und an der Menge.
5.  Teilen Sie die Artikel in die Klassen A, B und C ein. Verwenden Sie typische Grenzen (z.B. A-Güter: ~80% Wertanteil, B-Güter: ~15% Wertanteil, C-Güter: ~5% Wertanteil).
6.  Stellen Sie das Ergebnis grafisch in einem Diagramm dar (Lorenz-Kurve).
...
- **A-Güter:** Die Artikel 4, 2 und 9 sind A-Güter. Sie machen nur 30% der Artikelpositionen aus, binden aber rund 79% des Gesamtwertes. Diese Artikel erfordern die größte Aufmerksamkeit (z.B. genaue Bestandsführung, häufige Bestellungen, gute Lieferantenbeziehungen).
- **B-Güter:** Die Artikel 7, 3 und 1 sind B-Güter. Sie stellen die nächste Gruppe dar und erfordern eine standardmäßige Behandlung.
