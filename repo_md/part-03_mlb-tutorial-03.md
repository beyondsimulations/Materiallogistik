...
1.  **Materialbedarfsermittlung:** Führen Sie eine vollständige,
    mehrstufige Materialbedarfsermittlung (Stück-für-Stück) durch, um
    die Nettobedarfe und Bestellzeitpunkte für die Komponenten P1 und W1
    zu bestimmen.
2.  **Echelon Stock Berechnung:** Angenommen, am Ende von Woche 4 sind
    die physischen Lagerbestände wie folgt:
    `L1: 0, S1: 0, P1: 10, W1: 20`. Berechnen Sie für diesen Zeitpunkt
    den **Echelon Stock** (systemweiten Bestand) für das Wifi-Modul W1.
3.  **Nutzen des Echelon-Stock-Konzepts:** Erläutern Sie kurz den
    betriebswirtschaftlichen Nutzen des Echelon-Stock-Konzepts.
...
> ### Tipps und wichtige Formeln
>
> #### 1. Materialbedarfsermittlung (MRP)
>
> - **Grundlogik (Stufe für Stufe):**
>   1.  **Bruttobedarf ermitteln:** Für Endprodukte ist dies der
>       Primärbedarf. Für Komponenten ergibt er sich aus den *geplanten
>       Bestellungen* (Netto-Bedarf mit Vorlaufzeit) der übergeordneten
>       Stufe.
>   2.  **Nettobedarf berechnen:**
>       `Nettobedarf_t = Bruttobedarf_t - Lagerbestand_am_Anfang_der_Periode_t`.
>       Wenn das Ergebnis negativ oder null ist, ist der Nettobedarf 0.
>   3.  **Bestelltermin planen:** Der Auftrag zur Deckung des
>       Nettobedarfs in Periode `t` muss in Periode `t - Vorlaufzeit`
>       ausgelöst werden.
> - **Stück-für-Stück (Lot-for-Lot):** Die Bestellmenge entspricht exakt
>   dem Nettobedarf. Es wird kein Lagerbestand zur Losgrößenoptimierung
>   aufgebaut.
>
> #### 2. Echelon Stock
>
...
```
    ...
    Bestellplan (Stück-für-Stück):
    Artikel  Bedarfstermin  Nettobedarf  Bestelltermin
         W1              3           40              0
         L1              2           20              1
         P1              3           50              1
         W1              4           70              1
...
$E_{W1} = y_{W1} + y_{L1} + y_{S1}$ $E_{W1} = 20 + 0 + 0 = 20$

- Physischer Bestand P1: 10 Stück
- Bestand von L1 (enthält P1): 0 Stück

$E_{P1} = y_{P1} + y_{L1}$ $E_{P1} = 10 + 0 = 10$

In diesem speziellen Fall, da die Endproduktbestände Null sind,
entspricht der Echelon Stock dem physischen Bestand der Komponente. Wäre
der Bestand von L1 jedoch 5, wäre der Echelon Stock von W1:
$E_{W1} = 20 + 5 + 0 = 25$.

**3. Nutzen des Echelon-Stock-Konzepts:**

Das Echelon-Stock-Konzept ist für die **mehrstufige
Lagerkostenbewertung** entscheidend. Traditionell werden Lagerkosten auf
den physischen Bestand eines Artikels basierend auf dessen vollem Wert
berechnet. Dies führt zu einer Doppelzählung: Der Wert einer Komponente
(z.B. W1) ist bereits im Wert des Endprodukts (z.B. L1) enthalten. Wenn
man die Lagerkosten für beide Bestände separat berechnet, werden die
Kosten für das im Endprodukt gebundene Wifi-Modul doppelt gezählt.

Das Echelon-Konzept löst dies, indem es mit **marginalen Lagerkosten**
(`echelon holding costs`) arbeitet. Diese berechnen sich aus der
Kostendifferenz zur direkten Vorgängerstufe. Die Lagerkosten werden dann
auf den Echelon Stock angewendet. Dies führt zu einer korrekten, nicht
doppelt zählenden Bewertung der im gesamten Produktionssystem gebundenen
Kapital- und Lagerkosten und ermöglicht somit bessere
Losgrößenentscheidungen in mehrstufigen Systemen.

## Aufgabe 3: Rollierende Planung

Stellen Sie sich vor, Sie sind der Produktionsplaner aus Aufgabe 2. Der
6-Wochen-Plan wurde am Anfang von Woche 1 erstellt. Sie befinden sich
nun am Ende von Woche 2. Die Produktion für die Wochen 1 und 2 wurde
bereits freigegeben und kann nicht mehr geändert werden.
...
1.  **Eingefrorener Plan:** Welcher Teil des ursprünglichen
    ...
    Aufgabe 2) ist nun “eingefroren” und muss als gegeben hingenommen
    werden?
...
1.  **Eingefrorener Plan:** Der “Frozen”-Teil des Plans umfasst alle
    ...
    (Ende Woche 2) liegt. Gemäß der Lösung von Aufgabe 2 sind dies:

    - Bestellung für **Stecker S1** in Woche 0 (für Bedarf in Woche 1)
    - Bestellung für **Lampe L1** in Woche 1 (für Bedarf in Woche 2)
    - Bestellung für **Panel P1** in Woche -1 und 2 (für L1-Bedarf in P2
      und P4)
    - Bestellung für **Wifi-Modul W1** in Woche -2, 0 und 1.

    Alle Produktions- und Beschaffungsvorgänge, die bereits gestartet
    wurden (Bestellzeitpunkt \<= 2), sind fix. Die daraus resultierenden
    Zugänge und Bestände zu Beginn von Woche 3 sind die Ausgangsbasis
    für die Neuplanung.

2.  **Verlust der Optimalität:** Der ursprüngliche Plan basierte auf
    einer Stück-für-Stück-Politik, welche die Bedarfe exakt deckt. Die
    Erhöhung des Bedarfs für L1 in Woche 6 hat eine Kaskade von
    Effekten:

    - Der Bruttobedarf für P1 und W1 in Woche 5 (Vorlaufzeit von L1)
      erhöht sich.
    - Diese Bedarfsänderung wird an die Komponente W1 weitergereicht.
    - Die Losgrößenentscheidung für W1 im ursprünglichen Plan war darauf
      ausgelegt, die Bedarfe bis Woche 6 zu decken. Durch den neuen,
      höheren Bedarf in der Zukunft könnte es nun kostenoptimaler sein,
      eine frühere Bestellung für W1 (z.B. die in Woche 1 für den Bedarf
      in Woche 4) zu vergrößern, um die Bedarfe bis Woche 6 abzudecken
      und so einen Rüstvorgang zu sparen (falls eine andere
      Losgrößenregel als Stück-für-Stück verwendet würde).
    - Selbst bei Stück-für-Stück ändert sich die benötigte Bestellmenge
      für W1. Da die ursprüngliche Bestellung für den Bedarf in Woche 5
      (ausgelöst in P2) bereits “frozen” sein könnte, muss nun eine
      zusätzliche, evtl. kleine und ineffiziente Bestellung für den
      Mehrbedarf aufgegeben werden.
    - Fazit: Eine kleine Änderung in der Zukunft kann durch die
      komplexen, mehrstufigen Abhängigkeiten und Losgrößenlogiken die
      Optimalität des gesamten bestehenden Plans zunichtemachen.
...
3.  **Definitionen:**
    ...
    - **Frozen Zone:** Dies ist der unmittelbar bevorstehende Zeitraum
      ...
      der Produktion. Die Länge der Frozen Zone wird oft durch die
      kumulierte Vorlaufzeit der Produkte bestimmt.
