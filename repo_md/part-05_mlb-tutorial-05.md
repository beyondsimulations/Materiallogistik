...
3.  **Optimales Bestellniveau:** Bestimmen Sie das optimale
    Bestellniveau $S_{opt}$. Nehmen Sie zur Vereinfachung an, dass der
    Fehlmengenanteil direkt aus der Einheiten-Verlustfunktion der
    Normalverteilung über den gesamten Risikozeitraum $(r+L)$ abgeleitet
    werden kann. $S_{opt} = \mu_{r+L} + v_{opt} \cdot \sigma_{r+L}$,
    wobei $v_{opt}$ so gewählt wird, dass
    $G_Z^{(1)}(v_{opt}) \leq \frac{(1-\beta) \cdot r \cdot \mu_D}{\sigma_{r+L}}$.
    *Nutzen Sie die Approximation
    $G_Z^{(1)}(v) \approx \phi(v) - v \cdot (1-\Phi(v))$.*
...
>   gibt, muss der Wert für $v$ iterativ gesucht werden (z.B. durch
>   Ausprobieren oder mit einem Solver).
